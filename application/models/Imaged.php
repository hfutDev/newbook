<?php
   ...
   * 处理图片文件上传和图片剪切
   */
   class Application_Model_Imaged
   {
   	   ...
   	   protected $pre;
	   	...
	   	function __construct($path, $prefis)
	   	{
	   	...
	   	{
	   		$path = APPLICATION_PATH . '/../public/photo/' . $this->imagePath;
	   		...
	   		if ($this->myFile->receive()) {
	   			if ($thumbName = $this->thumbImage($fileName, $name)) {
	   	...
	   	{
	   		$uploadPath = APPLICATION_PATH . '/../public/photo/' . $this->imagePath . $name;
	   	 ...
	   	 * @return [string]  文件名
	   	 */
	   	public function thumbImage($filename, $name)
	   	{
	   		$imgInfo = array();
	   		$width = '285';
	   		$height = '385';
	   		$newSize = array();
	   		$extName = $this->getExtension($filename);
	   		$name = $this->pre . $name . "." . $extName;
	   		$path = APPLICATION_PATH . '/../public/photo/' . $this->imagePath;
	   		$newPath = APPLICATION_PATH . '/../public/photo/' . $this->imagePath . $name;
